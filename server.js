...
// ✅ KYC Submit - إرسال الصور لتليجرام وحفظ file_id في DB
app.post('/api/kyc/submit', upload.fields([{ name: 'passportPhoto', maxCount: 1 }, { name: 'personalPhoto', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        // ✅ إرسال الصور للأدمن في تيليجرام وحفظ file_id
        if (global.botInstance) {
            try {
                ...
                const msg1 = await global.botInstance.telegram.sendPhoto(
                    ...
                    { source: req.files.passportPhoto[0].buffer },
                    {
                        ...
                        ...Markup.inlineKeyboard([
                            [Markup.button.callback('✅ قبول', `approve_kyc_${userId}`), 
                             Markup.button.callback('❌ رفض', `reject_kyc_${userId}`)]
                        ])
                    }
                );
                ...
                console.log(`✅ صور KYC أرسلت للأدمن: ${userId}`);
            } catch(e) {
                console.error('KYC send error:', e.message);
            }
        }
        
        // ✅ حفظ file_id فقط في قاعدة البيانات
        const KycRequest = require('./models').KycRequest;
        await KycRequest.deleteOne({ userId, status: { $in: ['pending', 'rejected'] } });
        await KycRequest.create({
            userId, fullName,
            passportPhotoFileId: passportFileId || 'pending',
            personalPhotoFileId: personalFileId || 'pending',
...
bot.command('stats', async (ctx) => { try { const s = await db.getMarketStats(); await ctx.reply(`📊 💎${s.price} 📈${s.change24h?.toFixed(2)||0}% 🟢${s.buyOrders} 🔴${s.sellOrders}`); } catch(e) {} });
...
bot.command('admin', async (ctx) => {
    ...
    await ctx.reply(`👑 *لوحة الأدمن*\n🆔 توثيق: ${kyc.length}\n💰 سحب: ${w.length}\n\n📸 استخدم /kyc_list لعرض صور التوثيق`, {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
...
// ✅ KYC List - يعرض الصور من تيليجرام باستخدام file_id
        ...
        if (req.passportPhotoFileId && req.passportPhotoFileId !== 'pending') {
            try {
                await ctx.replyWithPhoto(req.passportPhotoFileId, {
                    ...
                    parse_mode: 'Markdown',
                    ...Markup.inlineKeyboard([
                        [Markup.button.callback('✅ قبول', `approve_kyc_${req.userId}`),
                         Markup.button.callback('❌ رفض', `reject_kyc_${req.userId}`)]
                    ])
                });
            } catch(e) {
                await ctx.reply(`📄 صورة هوية ${req.fullName} - غير متوفرة\nاستخدم: /approve_kyc_${req.userId} أو /reject_kyc_${req.userId}`);
            }
        } else {
            await ctx.reply(`⚠️ ${req.fullName} (\`${req.userId}\`) - الصور لم تصل بعد\nاستخدم: /approve_kyc_${req.userId} أو /reject_kyc_${req.userId}`);
        }
        
        // الصورة الشخصية
        if (req.personalPhotoFileId && req.personalPhotoFileId !== 'pending') {
            try {
                await ctx.replyWithPhoto(req.personalPhotoFileId, { caption: `📸 صورة شخصية - ${req.fullName}` });
            } catch(e) {}
        }
    }
});
...
// ✅ أوامر القبول والرفض النصية
bot.command(/approve_kyc_(.+)/, async (ctx) => {
    const userId = parseInt(ctx.message.text.split('_')[2]);
    if (!isAdmin(ctx.from.id)) return ctx.reply('⛔');
    try {
        const KycRequest = require('./models').KycRequest;
        ...
        if (global.botInstance) { try { await global.botInstance.telegram.sendMessage(userId, '🎉 *تم توثيق حسابك!*\n✅ يمكنك الآن التداول\n🎁 +100 CRYSTAL', { parse_mode: 'Markdown' }); } catch(e) {} }
    } catch(e) { await ctx.reply('❌ حدث خطأ'); }
});

bot.command(/reject_kyc_(.+)/, async (ctx) => {
    const userId = parseInt(ctx.message.text.split('_')[2]);
    if (!isAdmin(ctx.from.id)) return ctx.reply('⛔');
    try {
        const KycRequest = require('./models').KycRequest;
        const request = await KycRequest.findOne({ userId, status: 'pending' });
        if (!request) return ctx.reply('⚠️ لا يوجد طلب معلق');
        const result = await db.rejectKyc(request._id, ctx.from.id, 'صور غير واضحة');
        await ctx.reply(`❌ تم الرفض\n👤 \`${userId}\``, { parse_mode: 'Markdown' });
        if (global.botInstance) { try { await global.botInstance.telegram.sendMessage(userId, '⚠️ *تم رفض التوثيق*\nأعد الإرسال', { parse_mode: 'Markdown' }); } catch(e) {} }
    } catch(e) { await ctx.reply('❌ حدث خطأ'); }
});
...
bot.action(/approve_kyc_(.+)/, async (ctx) => {
    ...
    try {
        ...
        const result = await db.approveKyc(request._id, ctx.from.id);
        await ctx.editMessageCaption({ caption: `✅ *تمت الموافقة*\n👤 ${request.fullName}\n🎁 +100 CRYSTAL`, parse_mode: 'Markdown' });
        await ctx.reply(`✅ ${result.message}`);
        if (global.botInstance) { try { await global.botInstance.telegram.sendMessage(userId, '🎉 *تم توثيق حسابك!*\n✅ يمكنك الآن التداول\n🎁 +100 CRYSTAL', { parse_mode: 'Markdown' }); } catch(e) {} }
    } catch(e) { await ctx.reply('❌ حدث خطأ'); }
});
...
bot.action(/reject_kyc_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!request) { await ctx.reply('⚠️ الطلب غير موجود'); return; }
        const result = await db.rejectKyc(request._id, ctx.from.id, 'صور غير واضحة');
        await ctx.editMessageCaption({ caption: `❌ *تم الرفض*\n👤 ${request.fullName}`, parse_mode: 'Markdown' });
        if (global.botInstance) { try { await global.botInstance.telegram.sendMessage(userId, '⚠️ *تم رفض التوثيق*\nأعد الإرسال', { parse_mode: 'Markdown' }); } catch(e) {} }
    } catch(e) { await ctx.reply('❌ حدث خطأ'); }
});

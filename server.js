...
app.post('/api/order/create', async (req, res) => {
    try {
        const { user_id, type, price, amount, orderSubType, triggerPrice } = req.body;
...
app.post('/api/deposit', async (req, res) => {
    try {
        ...
        if (!['bnb', 'polygon'].includes(network)) return res.json({ success: false, message: '⚠️ الشبكة غير مدعومة. فقط BNB و Polygon' });
...
app.post('/api/channel/check', async (req, res) => {
    try {
        ...
        try { const chatMember = await bot.telegram.getChatMember(CHANNEL_ID, userId); isSubscribed = ['member', 'administrator', 'creator'].includes(chatMember.status); } catch(e) { return res.json({ success: false, message: '⚠️ تعذر التحقق' }); }
...
bot.start(async (ctx) => {
    try {
        ...
        let isSubscribed = false;
        try { const chatMember = await ctx.telegram.getChatMember(CHANNEL_ID, user.id); isSubscribed = ['member', 'administrator', 'creator'].includes(chatMember.status); } catch(e) {}
        
        const result = await db.registerUser(user.id, user.username || '', user.first_name || '', '', '', '', 'SD', '', referrer, 'ar', ctx.message?.chat?.id?.toString() || '', ctx.message?.from?.is_bot ? 'Bot' : 'User');
        if (isSubscribed && result.isNew) await db.giveChannelReward(user.id);
        ...
        if (result.isAdmin) msg = `👑 *أهلاً بالأدمن!*\n\n📦 رصيدك: ${sc.adminBalance.toLocaleString()} CRYSTAL\n💰 السعر: ${price} USDT`;
            ...
            msg += isSubscribed ? `\n\n🎁 *100 CRYSTAL* مكافأة القناة!` : `\n\n📢 انضم لقناتنا: ${CHANNEL_LINK}`;
        } else msg = `👋 *أهلاً بعودتك!*\n💰 السعر: ${price} USDT`;
        
        await ctx.reply(msg, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.webApp('💎 فتح المنصة', WEBAPP_URL)], [Markup.button.url('📢 القناة', CHANNEL_LINK)]]) });
...
bot.command('reward', async (ctx) => {
    try {
        let isSubscribed = false;
        try { const chatMember = await ctx.telegram.getChatMember(CHANNEL_ID, ctx.from.id); isSubscribed = ['member', 'administrator', 'creator'].includes(chatMember.status); } catch(e) {}
        if (!isSubscribed) return ctx.reply(`⚠️ اشترك أولاً!\n${CHANNEL_LINK}`);
        const result = await db.giveChannelReward(ctx.from.id);
...
bot.command('balance', async (ctx) => { try { const s = await db.getUserStats(ctx.from.id); if (!s) return ctx.reply('⚠️ سجل أولاً'); await ctx.reply(`💰 CRYSTAL: ${s.crystalBalance?.toFixed(2)||0}\n💵 USDT: ${s.usdtBalance?.toFixed(2)||0}`); } catch(e) {} });
...
bot.command('admin', async (ctx) => {
    ...
    await ctx.reply(`👑 *لوحة الأدمن*\n🆔 توثيق: ${kyc.length}\n💰 سحب: ${w.length}\n📤 إيداع يدوي: ${d.length}`, {
        ...
        ...Markup.inlineKeyboard([
            ...
            [Markup.button.callback(`📤 إيداع يدوي (${d.length})`, 'admin_deposits')]
        ])
    });
});
...
// ✅ أمر pending - السحوبات أولاً، الإيداعات اليدوية ثانياً
bot.command('pending', async (ctx) => {
    if (!isAdmin(ctx.from.id)) return ctx.reply('⛔');
    const w = await db.getPendingWithdraws(), d = await db.getPendingDeposits();
    if (!w.length && !d.length) return ctx.reply('📭 لا توجد طلبات معلقة');
    
    // ✅ السحوبات (تحتاج موافقة)
    if (w.length > 0) {
        for (const r of w) {
    ...
    // ✅ الإيداعات اليدوية (شبكات غير مدعومة تلقائياً)
    if (d.length > 0) {
        for (const r of d) {
            await ctx.reply(`📤 *إيداع يدوي*\n👤 \`${r.userId}\`\n💰 ${r.amount} USDT\n🌐 ${r.network}\n⚠️ شبكة غير مدعومة تلقائياً`, {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([[Markup.button.callback('✅ تأكيد يدوي', `confirm_deposit_${r._id}`)]])
            });
        }
    }
});

bot.command('fix', async (ctx) => { if (!isAdmin(ctx.from.id)) return; const f = await db.fixStuckOrders(), b = await db.validateBalances(); await ctx.reply(`🔧 ${f}\n✅ ${b.isValid?'سليم':'خطأ'}`); });
...
bot.action(/approve_kyc_(.+)/, async (ctx) => {
    ...
    if (req && global.botInstance) { try { await global.botInstance.telegram.sendMessage(req.userId, '🎉 تم توثيق حسابك! ✅'); } catch(e) {} }
});

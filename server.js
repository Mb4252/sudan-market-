...
bot.command('sell', async (ctx) => {
    ...
    const result = await db.createOffer(ctx.from.id, 'sell', currency, fiatAmount, price, paymentMethod, paymentDetails, '', '', '', 1, fiatAmount, true);
...
bot.command('buy', async (ctx) => {
    ...
    if (isNaN(fiatAmount) || fiatAmount <= 0) return ctx.reply('❌ مبلغ غير صحيح');
    if (isNaN(price) || price <= 0) return ctx.reply('❌ سعر غير صحيح');
    const result = await db.createOffer(ctx.from.id, 'buy', currency, fiatAmount, price, paymentMethod, paymentDetails, '', '', '', 1, fiatAmount, true);
    await ctx.reply(result.message + `\n💰 *رسوم التداول:* ${PLATFORM_TRADE_FEE} $`, { parse_mode: 'Markdown' });
});
